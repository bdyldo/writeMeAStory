...
class ModelBenchmark:
    ...
    async def run_single_benchmark(
        ...
        try:
            ...
            async for word in self.generator.generate_story_stream(
    ...
    async def run_performance_tests(self) -> Dict[str, Any]:
        ...
        test_cases = [
            ...
            (
                "The ancient forest was filled with mysterious creatures and hidden secrets",
                50,
                0.7,
            ),  # Long, very creative
            ("Write a story about", 15, 0.3),  # Short, moderate creativity
            (
                "The spaceship landed on the alien planet and the crew prepared to explore",
                40,
                0.6,
            ),  # Long, creative
        ]
        ...
        for i, (prompt, max_tokens, temperature) in enumerate(test_cases):
            ...
            if result["success"]:
                ...
                print(
                    f"✅ Success: {result['generated_length']} words in {result['generation_time']:.2f}s"
                )
            else:
    ...
    async def run_stress_test(self, concurrent_requests: int = 3) -> Dict[str, Any]:
        ...
        print(
            f"📈 Stress test completed: {successful_stress_tests}/{concurrent_requests} successful"
        )
    ...
    def validate_performance_thresholds(self) -> Dict[str, Any]:
        ...
        avg_time_check = {
            ...
            "passed": metrics.get("avg_generation_time", 0)
            <= thresholds["max_avg_generation_time"],
        }
        ...
        max_time_check = {
            ...
            "passed": metrics.get("max_generation_time", 0)
            <= thresholds["max_individual_generation_time"],
        }
        validation_results["threshold_checks"].append(max_time_check)

        # Determine overall pass/fail
        validation_results["passed"] = all(
            check["passed"] for check in validation_results["threshold_checks"]
        )

        # Generate recommendations
        if not avg_time_check["passed"]:
            validation_results["recommendations"].append(
                "Consider optimizing model inference time"
            )
        ...
        if not max_time_check["passed"]:
            validation_results["recommendations"].append(
                "Add request timeout handling for slow requests"
            )

        return validation_results

    def generate_report(
        self, stress_results: Dict[str, Any], validation_results: Dict[str, Any]
    ) -> str:
        ...
        for check in validation_results["threshold_checks"]:
            ...
            report_lines.append(
                f"  • {check['metric']}: {check['value']:.2f} (threshold: {check['threshold']}) {status}"
            )
