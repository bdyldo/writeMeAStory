...
class ProperTransformer(nn.Module):
    ...
    def generate(
        self, tokens: Tensor, max_tokens: int = 50, temperature: float = 1.0
    ) -> Tensor:
    ...
    for idx, sequence in enumerate(train_data):
        ...
        if idx % val_index == 0:
            avg_train_loss = round(
                train_batch_loss / val_index if idx != 0 else train_batch_loss, 6
            )
            train_batch_losses.append(avg_train_loss)
            train_batch_loss = 0.0

            print(
                f"Batch: {idx} | Seq Length: {sequence.shape[1]} | "
...
def complete_transformer(model, tokenizer, prefix: str, num_tokens=64, temperature=0.0):
    ...
    output = model.generate(
        input_tokens, max_tokens=num_tokens, temperature=temperature
    )
...
if __name__ == "__main__":
    ...
    # Model arguments
    parser.add_argument(
        "--d_model", type=int, default=128, help="Model dimension (replaces hidden_dim)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pretrained_weights", type=str, help="Path to pre-trained weights (.pt file)"
    )
    parser.add_argument(
        "--save_model_path",
        type=str,
        default="transformer_proper.pt",
    ...
    for prompt in test_prompts:
        completion = complete_transformer(
            model, tokenizer, prompt, num_tokens=32, temperature=0.6
        )
        print(f"Test: {prompt}")
